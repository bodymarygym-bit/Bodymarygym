...
// Crear carpetas de archivos automáticamente
const uploadsDir = path.join(__dirname, "uploads");
const soundsDir = path.join(__dirname, "sounds");

/*====================================================
    ARCHIVO DE MÚSICA DEL SITIO
    La música subida por el administrador vive en /sounds.
====================================================*/
...
/* Migra la música antigua de /uploads a /sounds una sola vez. */
if (fs.existsSync(OLD_MUSIC_FILE)) {
    try {
        if (!fs.existsSync(MUSIC_FILE)) {
            fs.renameSync(OLD_MUSIC_FILE, MUSIC_FILE);
        } else {
            fs.unlinkSync(OLD_MUSIC_FILE);
        }
    } catch(error) {
        console.warn("No se pudo migrar site-music.mp3:", error.message);
    }
}
...
app.post(
    ...
    (req, res) => {
        ...
        try {
            ...
            /*=========================================
                ELIMINAR MÚSICA ANTERIOR Y TEMPORALES
                Conservamos únicamente la música subida como
                /sounds/site-music.mp3. Los sonidos del sistema
                (Alarma.mp3, share.mp3, like.mp3) no se tocan.
            =========================================*/

            fs.readdirSync(soundsDir).forEach(nombre => {
                if(
                    nombre === "site-music.mp3" ||
                    /^music-temp-\d+\.mp3$/i.test(nombre)
                ){
                    const ruta = path.join(soundsDir, nombre);
                    try {
                        if(fs.existsSync(ruta)){
                            fs.unlinkSync(ruta);
                        }
                    } catch(error){
                        console.warn("No se pudo eliminar música anterior:", error.message);
                    }
                }
            });
